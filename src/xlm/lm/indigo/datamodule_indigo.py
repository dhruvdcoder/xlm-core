...
class IndigoEmptyDataset(IterableDataset):
    def __init__(
        ...
        self.uniform = uniform # true if yield all examples w same fixed length
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_examples):
            L = fixed_len if self.uniform else int(
                self.rng.integers(self.min_len, self.max_len + 1)
            )
            ids = self.rng.choice(self.normal_ids, size=L, replace=True).tolist()
...
def build_insertion_trajectory_random(
    ...
    R = torch.tensor([[0, 1], [-1, 0]], dtype=torch.int8)  # initial relative matrix
    ...
    rebuild_sorted()
    absolute_positions = torch.tensor([a for a, _ in current], dtype=torch.long)
...
class DefaultIndigoCollator(Collator):
    ...
    def __call__(
        ...
        targets = [self._truncate(list(e.get("target_ids") or e.get("input_ids") or [])) for e in examples]
        
        batch_size = len(targets)
        max_tgt_len = max(len(t) for t in targets) if targets else 1
        pad_id = self._pad

        target_ids = torch.full((batch_size, max_tgt_len), pad_id, dtype=torch.long)
        ...
        # Pad sequences
        max_steps = max(w.size(0) for w in word_label_list) if word_label_list else 1
        word_labels = torch.full((batch_size, max_steps), pad_id, dtype=torch.long)
        word_labels_mask = torch.zeros((batch_size, max_steps), dtype=torch.bool)
        pointer_labels = torch.full((batch_size, max_steps), -100, dtype=torch.long)
        pointer_labels_mask = torch.zeros((batch_size, max_steps), dtype=torch.bool)
        ...
        order_indices = torch.full((batch_size, max_order), -100, dtype=torch.long)
        for i, oi in enumerate(order_list):
            order_indices[i, : oi.size(0)] = oi

        # Pad relative matrices 
        rel_size = max_tgt_len + 2
        relative_matrix = torch.zeros((batch_size, rel_size, rel_size), dtype=torch.int8)
        absolute_positions = torch.zeros((batch_size, rel_size), dtype=torch.long)
...
class IndigoSeq2SeqCollator:
    ...
    def _truncate(self, seq: List[int], max_len: Optional[int]) -> List[int]:
        if max_len is not None and self.truncate == "block" and len(seq) > max_len:
            return seq[:max_len]
        return seq
    ...
    def __call__(
        ...
        # TODO (URV): Implement the collator.
        if not examples:
            raise ValueError("Empty examples passed to seq2seq collator.")
        ...
        # Source padding
        max_src_len = max(len(s) for s in sources) if sources else 1
...
class IndigoSeq2SeqPredCollator(IndigoSeq2SeqCollator):
    ...
    def __call__(
        ...
        target_ids = torch.full((batch_size, max_tgt), pad_id, dtype=torch.long)
        target_attention_mask = torch.zeros_like(target_ids, dtype=torch.bool)
...
def print_batch_indigo(
    ...
    for i in range(show):
        ...
        if batch.get("input_ids") is not None:
            ...
            src_mask = batch.get("attention_mask", torch.ones_like(src_ids)).bool()
            src_tokens = [tokenizer.decode([int(t)]) for t in src_ids[src_mask]]
