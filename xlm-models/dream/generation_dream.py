...
class DreamGenerationMixin:
    ...
    def _prepare_generated_length(
        ...
        if generation_config.max_new_tokens is not None:
            ...
            if (
                generation_config.max_length
                == DreamGenerationConfig().max_length
            ):
                generation_config.max_length = (
    ...
    def _prepare_generation_config(
        self,
        generation_config: Optional[DreamGenerationConfig],
        **kwargs: Dict,
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        """
        Args:
            inputs: The input_ids tokens to the model with padding on the left.
            kwargs:
                attention_mask: The attention mask for the input_ids
                max_new_tokens: The maximum number of tokens to generate
                output_history: Whether to output the history of the generation
                return_dict_in_generate: Whether to return a dictionary of generation results
                steps: The number of steps to take in the diffusion process
                temperature: The temperature for the generation
                top_p: The top_p for the generation
                top_k: The top_k for the generation
                alg: The algorithm to use for the generation
                alg_temp: The temperature for the alg
        """
        ...
        # convert special tokens into tensors to become compile friendly
    ...
    def _sample(
        ...
        histories = (
            [] if (return_dict_in_generate and output_history) else None
        )
        ...
        for i in range(steps):
            ...
            if alg == "origin":
                ...
                if number_transfer_tokens > 0:
                    ...
                    x_ = (
                        torch.zeros_like(
                            x, device=self.device, dtype=torch.long
                        )
                        + mask_token_id
                    )
                    ...
                    x[row_indices, transfer_index] = x_[
...
class DreamLogitsHook:
    """Shift logits for masked diffusion (prd2)."""
