...
class FlexMDMModel(Model):
    ...
    def __init__(
        ...
        if self.len_predict_type == "distribution":
            if self.len_head_type == "linear":
                self.len_pred = (
                    DDitFinalLayer(  # len_predict_type == "distribution"
                        d_model,
                        max_length,
                        self.d_cond,
                        layer_norm_eps,
            ...
            elif self.len_head_type == "mlp":
                self.len_pred = DDitFinalLayerForClassification(
                    d_model,
                    max_length,
                    self.d_cond,
                    layer_norm_eps,
                ...
                raise ValueError(
                    f"Invalid length head type: {self.len_head_type}"
                )
        elif self.len_predict_type == "expectation":
            self.len_pred = ScalarLengthHead(d_model, max_length, self.d_cond)
        else:
            raise ValueError(
    ...
    def forward(
        ...
        """
        B, L = x.shape
        # Don't need this if using eos token
        # x = torch.cat(
        #    [
        #        x,
        #        torch.full((B, 1), self.padding_idx, device=x.device, dtype=torch.int64),
        #    ],
        #    dim=-1,
        # )
        ...
        length_posterior = self.len_pred(x, c)  # (B, L)
