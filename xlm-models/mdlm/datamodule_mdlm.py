...
def mdlm_single_segment_collate_fn(
    ...
    # only difference between mlm and mdlm
    ...
    # here we use noise_rate to mask instead of t directly like in mlm
    mask = (
        torch.rand(batch_size, max_seq_len, device=total_noise.device)
...
def prepare_prefix_suffix_ids(
    ...
    for i, (_prefix_ids, _suffix_ids) in enumerate(
        ...
        _mask = torch.rand(len(temp), device=total_noise.device) < -(
...
class DefaultMDLMCollator(Collator):
    ...
    def __call__(
        ...
        return mdlm_single_segment_collate_fn(
...
class MDLMSeq2SeqTrainCollator(Collator):
...
class MDLMSeq2SeqPredCollator(Collator):
    """Input contains only the prefix and target_ids contain only the suffix if present.

    How is this different from MDLMSeq2SeqTrainCollator?
        -  MDLMSeq2SeqTrainCollator's input_ids contain the joined sequence and target_ids also contain the target for the whole sequence. But MDLMSeq2SeqPredCollator's input_ids contain only the prefix and target_ids contain only the suffix if present.
...
def print_batch_mdlm(
    ...
    if "t" in batch:
        print("t:")
        print(batch["t"][0])
    if "noise_rate" in batch:
        print("noise_rate:")
        print(batch["noise_rate"][0])
    if "total_noise" in batch:
        print("total_noise:")
